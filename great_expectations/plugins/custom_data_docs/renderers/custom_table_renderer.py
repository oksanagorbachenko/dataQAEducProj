...
from great_expectations.render.renderer import CallToActionRenderer
...
class CustomTableRenderer(SiteIndexPageRenderer):
    ...
    def _generate_validation_results_link_table(cls, index_links_dict):
        ...
        for dict_ in validation_link_dicts:
            ...
            # match = re.search('.+(\/)(.*?).html', str(dict_.get("batch_kwargs")['link'][-1-i]))
            # file = str(dict_.get("batch_kwargs")['link'][-1-i]).replace('https://d35b8h21obf2t9.cloudfront.net/','').replace(match.group(2)+'.html','')
            # objs = list(bucket.objects.filter(Prefix=file+'skip.txt'))
            # print('s3://ipa-data-qa/'+file+'skip.txt')
            # if not (len(objs) > 0 and objs[0].key == file+'skip.txt'):
            #     i = i + 1
